...
class GeminiProvider(ILLMProvider):
    """
    Summary: LangChain-based Google Gemini provider for chat response generation.
    Includes automatic retry on 429 rate-limit errors with a short backoff.
    """
    ...
    async def generate_response(
        ...
        for attempt in range(_MAX_RETRIES + 1):
            try:
                ...
                logger.error("Gemini failed to generate response: %s", exc)
                raise LLMException(
